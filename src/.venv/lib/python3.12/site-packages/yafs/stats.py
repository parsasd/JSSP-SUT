...
class Stats:
    """
    Helper class to analyse simulation results stored in CSV files.

    By default, it loads ``result.csv`` and ``result_link.csv`` generated
    by :class:`yafs.metrics.Metrics`.
    """

    def __init__(self, defaultPath: str = "result"):
        """
        Parameters
        ----------
        defaultPath:
            Base path for the CSV results (without extension).
        """
        self.df_link = pd.read_csv(defaultPath + "_link.csv")
        self.df = pd.read_csv(defaultPath + ".csv")

    def bytes_transmitted(self) -> float:
        """Return the total number of bytes transmitted over the network."""
        return self.df_link["size"].sum()

    def count_messages(self) -> int:
        """Return the total number of link-level records (messages/hops)."""
        return len(self.df_link)

    def utilization(self, id_entity, total_time, from_time: float = 0.0):
        """
        Compute utilization of a given DES entity over ``total_time``.
        """
    ...
    def compute_times_df(self) -> None:
        """Compute and cache all time-related metrics in ``self.df``."""
    ...
    def times(self, time, value: str = "mean"):
        """
        Aggregate time metrics per message.
        """
    ...
    def average_loop_response(self, time_loops):
        """
        Compute the total average response time of each message loop.

        Notes
        -----
        No check is performed on the existence of each loop; this is the
        user's responsibility.
        """
        if "time_response" not in self.df.columns:
            self.compute_times_df()

        # It is not strictly necessary to keep "count", but it is useful.
        resp_msg = self.df.groupby("message").agg(
            {"time_total_response": ["mean", "count"]}
        )
    ...
    def get_watt(self, totaltime, topology, by=Metrics.WATT_SERVICE):
        """
        Compute energy consumption in watts.
        ...
        topology:
            Topology object providing node information.
        by:
            Either :data:`Metrics.WATT_SERVICE` (energy by service time)
            or :data:`Metrics.WATT_UPTIME` (energy by uptime).
        """
        results = {}
        nodeInfo = topology.get_info()
        if by == Metrics.WATT_SERVICE:
            # Activity time (service time).
